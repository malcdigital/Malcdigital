...
class CandlestickScanner:
    ...
    # -- context -------------------------------------------------------------
    def _trend_context(self, bars: pd.DataFrame) -> pd.Series:
    ...
    # -- entry point ---------------------------------------------------------
    def scan(self, bars: pd.DataFrame, lookback: int = 10) -> list[Pattern]:
        ...
        trend = self._trend_context(bars)
        ...
        for i in range(start, len(bars)):
            ctx = float(trend.iloc[i])
            vol = float(vol_ratio.iloc[i])
            for detect in detectors:
                try:
                    hit = detect(bars, i, atr_series, ctx, vol)
    ...
    # -- single-bar patterns -------------------------------------------------
    def _hammer(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        c = _geometry(bars, i, atr_s)
        ...
        return Pattern(
            "Hammer", BULLISH, bars.index[i], i, strength, 1,
    ...
    def _hanging_man(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        c = _geometry(bars, i, atr_s)
        ...
        return Pattern(
            "Hanging Man", BEARISH, bars.index[i], i, strength, 1,
            f"Hammer shape after a {ctx:.1f}-ATR advance: selling pressure appearing "
            "inside an uptrend.",
        )

    def _inverted_hammer(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        c = _geometry(bars, i, atr_s)
        ...
        return Pattern(
            "Inverted Hammer", BULLISH, bars.index[i], i, strength, 1,
            "Long upper shadow after a decline: buyers probed higher. Needs an "
            "up-close next session to confirm.",
        )

    def _shooting_star(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        c = _geometry(bars, i, atr_s)
        if ctx < 0.6 or c.body_pct > 0.34:
            return None
        if c.upper_shadow < 2.0 * c.body or c.upper_shadow <= 0:
    ...
    def _doji(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        ...
        return Pattern(
            "Doji", direction, bars.index[i], i, strength, 1,
            f"Open and close nearly equal after a {which}: the prevailing move has "
            "stalled into indecision.",
        )

    def _marubozu(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        c = _geometry(bars, i, atr_s)
        ...
        return Pattern(
            "Marubozu", direction, bars.index[i], i, strength, 1,
            f"Full-bodied bar with almost no shadows: {side} controlled the entire "
            "session open to close.",
        )

    # -- two-bar patterns ----------------------------------------------------
    def _bullish_engulfing(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        prev, cur = _geometry(bars, i - 1, atr_s), _geometry(bars, i, atr_s)
        ...
        return Pattern(
            "Bullish Engulfing", BULLISH, bars.index[i], i, strength, 2,
    ...
    def _bearish_engulfing(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        prev, cur = _geometry(bars, i - 1, atr_s), _geometry(bars, i, atr_s)
    ...
    def _piercing_line(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        ...
        strength = _clamp(0.38 + 0.25 * _clamp((penetration - 0.5) / 0.5)
                          + 0.15 * _clamp((vol - 1.0) / 1.5))
        return Pattern(
            "Piercing Line", BULLISH, bars.index[i], i, strength, 2,
            f"Gapped down then closed {penetration:.0%} back into the prior red body: "
            "the gap was bought.",
        )

    def _dark_cloud_cover(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        prev, cur = _geometry(bars, i - 1, atr_s), _geometry(bars, i, atr_s)
        ...
        strength = _clamp(0.38 + 0.25 * _clamp((penetration - 0.5) / 0.5)
                          + 0.15 * _clamp((vol - 1.0) / 1.5))
        return Pattern(
            "Dark Cloud Cover", BEARISH, bars.index[i], i, strength, 2,
            f"Gapped up then closed {penetration:.0%} back into the prior green body: "
            "the breakout failed intraday.",
        )

    def _bullish_harami(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        prev, cur = _geometry(bars, i - 1, atr_s), _geometry(bars, i, atr_s)
        ...
        strength = _clamp(0.30 + 0.20 * _clamp(1.0 - cur.body / max(prev.body, 1e-9))
                          + 0.15 * _clamp(-ctx / 3.0))
        return Pattern(
            "Bullish Harami", BULLISH, bars.index[i], i, strength, 2,
            "A small body sits inside the prior large red body: selling momentum has "
            "contracted sharply.",
        )

    def _bearish_harami(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        prev, cur = _geometry(bars, i - 1, atr_s), _geometry(bars, i, atr_s)
        ...
        strength = _clamp(0.30 + 0.20 * _clamp(1.0 - cur.body / max(prev.body, 1e-9))
                          + 0.15 * _clamp(ctx / 3.0))
        return Pattern(
            "Bearish Harami", BEARISH, bars.index[i], i, strength, 2,
            "A small body sits inside the prior large green body: buying momentum has "
            "contracted sharply.",
        )

    def _tweezer_bottom(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        prev, cur = _geometry(bars, i - 1, atr_s), _geometry(bars, i, atr_s)
        ...
        return Pattern(
            "Tweezer Bottom", BULLISH, bars.index[i], i, strength, 2,
            "Two sessions rejected from an identical low: a precise level buyers are "
            "defending.",
        )

    def _tweezer_top(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        prev, cur = _geometry(bars, i - 1, atr_s), _geometry(bars, i, atr_s)
        ...
        strength = _clamp(0.28 + 0.18 * _clamp(ctx / 3.0) + 0.12 * _clamp((vol - 1.0) / 1.5))
        return Pattern(
            "Tweezer Top", BEARISH, bars.index[i], i, strength, 2,
            "Two sessions rejected from an identical high: a ceiling sellers are "
            "defending.",
        )

    # -- three-bar patterns --------------------------------------------------
    def _morning_star(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        a, b, c = (_geometry(bars, i - 2, atr_s), _geometry(bars, i - 1, atr_s),
                   _geometry(bars, i, atr_s))
        ...
        return Pattern(
            "Morning Star", BULLISH, bars.index[i], i, strength, 3,
    ...
    def _evening_star(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        a, b, c = (_geometry(bars, i - 2, atr_s), _geometry(bars, i - 1, atr_s),
                   _geometry(bars, i, atr_s))
        ...
        strength = _clamp(0.52 + 0.22 * _clamp((giveback - 0.5) / 0.5)
                          + 0.16 * _clamp((vol - 1.0) / 1.5) + 0.10 * _clamp(ctx / 3.0))
        return Pattern(
            "Evening Star", BEARISH, bars.index[i], i, strength, 3,
            f"Strong up-bar, an exhausted gap-up star, then a decline giving back "
            f"{giveback:.0%} of the advance: a three-session top.",
        )

    def _three_white_soldiers(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        g = [_geometry(bars, i - k, atr_s) for k in (2, 1, 0)]
        ...
        return Pattern(
            "Three White Soldiers", BULLISH, bars.index[i], i, strength, 3,
    ...
    def _three_black_crows(self, bars, i, atr_s, ctx, vol) -> Pattern | None:
        g = [_geometry(bars, i - k, atr_s) for k in (2, 1, 0)]
        ...
        strength = _clamp(0.55 + 0.20 * _clamp((vol - 1.0) / 1.5)
                          + 0.15 * _clamp(sum(x.body / max(x.atr, 1e-9) for x in g) / 3 - 0.6))
        return Pattern(
            "Three Black Crows", BEARISH, bars.index[i], i, strength, 3,
