...
def test_diagnostics_reports_a_structured_verdict():
    ...
    # Whatever the network state, the verdict must be a definite boolean.
    assert isinstance(diagnosis.live_data_available, bool)
    assert all(isinstance(c.ok, bool) for c in diagnosis.checks)


def test_diagnostics_marks_price_feeds_as_fatal():
